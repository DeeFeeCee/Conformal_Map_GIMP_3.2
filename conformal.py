...
GRADIENT_ID_MAP = {0: "HSV", 1: "red-blue", 2: "black-white", 3: "white-black", 4: "custom"}
...
def _ensure_vendored_package_path(package_name, package_path):
...
def _ensure_vendored_sympy_path():
    ...
    _ensure_vendored_package_path("mpmath", VENDORED_MPMATH_PATH)
    _ensure_vendored_package_path("sympy", VENDORED_SYMPY_PATH)
...
class _FormulaMathNamespace:
    """Expose math.* syntax while preferring complex-capable implementations."""
...
MATH_NAMESPACE = {
    # `math.exp(z)` is accepted formula syntax, so it must not fail merely
    # because Python's math module is real-only. Use the cmath equivalent
    # when available while retaining math-only helpers such as floor.
    "math": _FormulaMathNamespace(),
    "cmath": cmath,
    "complex": complex,
    "abs": abs,
    "min": min,
...
class ConformalRenderer:
    ...
    def __init__(
        ...
        # Coordinates identify pixel centers.  This gives each canvas axis a
        # full-width/full-height period, so loop tiling joins exactly at the
        # canvas boundary instead of drifting by one edge pixel.
        self._domain_sx = self.width / (self.domain_xr - self.domain_xl)
        self._domain_sy = self.height / (self.domain_yt - self.domain_yb)
        self._source_sx = self.width / (self.source_xr - self.source_xl)
        self._source_sy = self.height / (self.source_yt - self.source_yb)
    ...
    def _sympy_expression_to_python(expression):
        ...
        # Keep the symbolic vocabulary aligned with the formula vocabulary.
        # In particular, `e` is supplied by both math and cmath, but used to
        # be parsed as a free variable here. A bare free variable cannot be
        # inverted, which made otherwise valid formulas such as `e^z` fall
        # back to forward splatting.
        local_dict = {
            "z": z,
            "w": w,
            "i": sp.I,
        ...
        # SymPy uses the same names for the analytic functions exposed to
        # formulas by math/cmath (including asin, erf, gamma, and LambertW).
        # Adding the intersection prevents a permitted formula name from being
        # mistaken for an unknown symbol simply because it was not in a small
        # hand-maintained list.
        for _name in MATH_NAMESPACE:
            if _name.isidentifier() and hasattr(sp, _name):
        ...
        # The formula evaluator permits math.sin(z) and cmath.sin(z), too.
        # They have identical symbolic meanings, so remove just these approved
        # module qualifiers before handing the expression to SymPy.
        expression = re.sub(r"\b(?:math|cmath)\.([A-Za-z_]\w*)", r"\1", expression)
    ...
    @staticmethod
    def _sympy_inverse_to_python(expression):
        """Print a symbolic inverse using formula syntax without inventing sqrt()."""
        _ensure_vendored_sympy_path()
        import sympy as sp
        from sympy.printing.str import StrPrinter

        class _FormulaPrinter(StrPrinter):
            def _print_Pow(self, expr, rational=False):
                if expr.exp == sp.S.Half:
                    return f"({self._print(expr.base)}) ** (0.5)"
                return super()._print_Pow(expr, rational=rational)

        return _FormulaPrinter().doprint(expression).replace("I", "1j")
    ...
    def symbolic_inverse_code(code):
        ...
        for solution in solutions:
            inverse_expr = ConformalRenderer._sympy_inverse_to_python(solution)
            inverse_code = f"z = ({inverse_expr})"
            # An inverse renderer samples an output point w, so the relevant
            # condition is f(g(w)) == w. Requiring g(f(z)) == z rejects every
            # valid principal branch of a multi-valued inverse (for example a
            # quadratic rational map), even though that branch renders the
            # output correctly. The output-side check still rejects lossy
            # expressions such as sqrt(z**2).
            if ConformalRenderer._is_output_valid_inverse(code, inverse_code):
                return inverse_code
        return None

    @staticmethod
    def _check_inverse_identity(code, inverse_code, samples, recover_source):
        """Check either g(f(z)) == z or f(g(w)) == w for sample points."""
        try:
    ...
    def _is_source_recovering_inverse(code, inverse_code):
        ...
        # Include both half-planes to catch sqrt(z**2), whose principal root
        # discards which side supplied the point.
        # Chosen are points between origin & 1, not between center & corners.
        samples = (-0.5 - 0.5j, -0.5 + 0.5j, 0.5 - 0.5j, 0.5 + 0.5j)
        return ConformalRenderer._check_inverse_identity(code, inverse_code, samples, recover_source=True)

    @staticmethod
    def _is_output_valid_inverse(code, inverse_code):
        """Return whether the proposed branch maps representative outputs back."""
        # Avoid branch cuts and singularities while exercising all quadrants.
        samples = (-0.5 - 0.5j, -0.5 + 0.5j, 0.5 - 0.5j, 0.5 + 0.5j)
        return ConformalRenderer._check_inverse_identity(code, inverse_code, samples, recover_source=False)
    ...
    def _validate_gradient_setting(self):
        ...
        if gradient_name in ("hsv", "red-blue", "black-white", "white-black"):
            return
    ...
    def _evaluate_point(self, z):
        ...
        if valid:
            try:
                # Use source coordinates where the center-to-selected-edge distance is 1 unit.
                logw = cmath.log(w)
                arg = logw.imag
                if arg < 0.0:
                    arg += self._two_pi
    ...
    def _sample_mapped_pixel(self, source_pixels, sx, sy, use_abyss=True):
        ...
        # All abyss policies apply only after a coordinate leaves the source image.
        # Previously Transparent (and the solid-color policies) fell
        # through to their abyss value even for in-bounds samples, making the
        # entire transformed layer blank or a single solid color.
    ...
    def _safe_floor_to_int(self, value):
        try:
    ...
    def _source_coord_to_pixel(self, z):
        sx = self._safe_floor_to_int((z.real - self.source_xl) * self._source_sx)
        sy = self._safe_floor_to_int((self.source_yt - z.imag) * self._source_sy)
        return sx, sy

    def _domain_coord_to_pixel(self, w):
        ox = self._safe_floor_to_int((w.real - self.domain_xl) * self._domain_sx)
        oy = self._safe_floor_to_int((self.domain_yt - w.imag) * self._domain_sy)
        return ox, oy
    ...
    def _render_inverse_mapped(self, source_pixels, progress_cb=None):
        ...
        for row in range(self.height):
            ...
            imag = self.domain_yt - ((row + 0.5) / self._domain_sy)
            for col in range(self.width):
                w = (col + 0.5) / self._domain_sx + self.domain_xl + 1j * imag
    ...
    def _accumulate_forward_pixel(self, accum, source_pixels, sx, sy, z):
        # Forward splatting needs only the mapped coordinate.  Do not route it
        # through analysis evaluation, which rejects w == 0 because log(0) is
        # undefined even though zero is a valid destination coordinate.
        valid, w = self._evaluate_mapping(z)
        if not valid:
            return None
    ...
    def _forward_center_outputs(self, accum, source_pixels, progress_cb=None):
        ...
        for sy in range(self.height):
            imag = self.source_yt - ((sy + 0.5) / self._source_sy)
            for sx in range(self.width):
                z = ((sx + 0.5) / self._source_sx + self.source_xl) + 1j * imag
    ...
    def _render_forward_mapped(self, source_pixels, progress_cb=None):
        ...
        if self.transform_precision <= 0:
            ...
            progress = 0.0
            for sy in range(self.height):
                imag_base = self.source_yt - ((sy + 0.5) / self._source_sy)
                for sx in range(self.width):
                    if self._forward_pixel_is_surrounded(center_outputs, sx, sy):
                        progress += len(offsets)
                        continue
                    for ox_off, oy_off in offsets:
                        z = ((sx + 0.5 + ox_off) / self._source_sx + self.source_xl) + 1j * (imag_base - (oy_off / self._source_sy))
    ...
    def render(self, source_pixels=None, progress_cb=None):
        ...
        for row in range(self.height):
            ...
            for col in range(self.width):
                ...
                z = (col + 0.5) / self._domain_sx + self.domain_xl + 1j * imag
...
def _show_dialog(procedure, config, width, height):
    ...
    for key, label in [("HSV", "HSV"), ("red-blue", "Red-Blue"), ("black-white", "Black-White"), ("white-black", "White-Black"), ("custom", "Custom")]:
...
class ConformalPlugin(Gimp.PlugIn):
    ...
    def do_create_procedure(self, name):
        ...
        choices_gradient.add("red-blue", 1, _("Red-Blue"), "Red to blue")
        choices_gradient.add("black-white", 2, _("Black-White"), "Black to white")
